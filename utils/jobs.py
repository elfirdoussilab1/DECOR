...
class Jobs:
    ...
    def _run(topdir, name, seed, device, command):
        ...
        with utils.Context(name, "info"):
            ...
            utils.trace((" ").join(shlex.quote(arg) for arg in args))
            cmd_res = subprocess.run(args, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            if cmd_res.returncode == 0:
                utils.info("Experiment successful")
            else:
                utils.warning("Experiment failed")
