...
def plot_comparison_loss(A, B, gamma, num_nodes, num_dim, sigma_cdp, sigma_cor, c_clip, num_gossip=1, num_iter = 1000, delta = 1e-4):
    """
    ...
    Args:
        ...
        delta (float): parameter delta for privacy
    ...
    epsilon_global = dp_account.rdp_compose_convert(num_iter=num_iter, rdp_eps=eps_rdp_iteration, delta= delta)
    ...
    ax.set_xlabel('iteration')
    ax.set_ylabel('loss')
    ax.set_title(f"loss with user privacy {epsilon_global}")
    ax.legend()
...
def find_best_params(A, B, num_nodes, num_dim, gamma_grid, c_clip_grid, sigma_cor_grid, max_loss, target_eps, num_gossip=1, num_iter= 1000):
    """
    ...
    # Initilization of the results dataframe
    data = [{"gamma": gamma_grid[0], "c_clip": c_clip_grid[0], "sigma_cdp": -1, "sigma_cor": -1, "eps": -1, "loss_cdp":10, "loss_cor": 10}]
    result = pd.DataFrame(data)
    # Loop over learning rates
    for gamma in gamma_grid:
        print(f"loop gamma {gamma}")
        # Loop over gradient clipping
        for c_clip in c_clip_grid:
            print(f"loop clip {c_clip}")
            # sigma_ldp
            ...
            for sigma_cdp in sigma_cdp_grid:
                ...
                if len(all_sigma_cor) != 0: # Not empty
                    for sigma_cor in all_sigma_cor:
                        ...
                        if errors_centr[-1] <= max_loss and result["loss_cor"].iloc[-1] - min(errors_cor[950:]) > 0:
                            new_row = { "gamma": gamma,
                                        "c_clip": c_clip,
                                        "sigma_cdp":sigma_cdp, 
                                        "sigma_cor": sigma_cor, 
